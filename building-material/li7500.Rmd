---
title: "Using fluxible with the Li7500"
output: github_document
# output: rmarkdown::html_vignette
# vignette: >
#   %\VignetteIndexEntry{Preparing the data for fluxible}
#   %\VignetteEngine{knitr::rmarkdown}
#   %\VignetteEncoding{UTF-8}
# bibliography: biblio_phd_zot.bib
# csl: emerald-harvard.csl
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
options(tidyverse.quiet = TRUE)
```

In this vignette, we will see how to read the data from the setup described in @halbritterPlantTraitVegetation2024 and compare the output calculated with fluxible with the published data.


## Importing and reading the files

First, we import the data from OSF with `dataDownloader::get_file`.

```{r osf-import, include = TRUE}
library(dataDownloader)

get_file("gs8u6",
         "rawData/raw_c-flux",
         "rawC-flux.zip",
         "vignettes/ex_data/li7500")

unzip("vignettes/ex_data/li7500/rawC-flux.zip",
      exdir = "vignettes/ex_data/li7500/rawC-flux")
```

Then we import the data with `licoread::import7500`.
```{r reading, message=FALSE}
library(licoread)
library(tidyverse)
pftc5_data_raw <- import7500(
  "vignettes/ex_data/li7500/rawC-flux/rawData/raw_tent-flux",
  plotinfo_names = c("site", "treatment", "date", "plot_id", "trial")
)
```

The `licoread::import7500` function provides `f_fluxid`, `f_start` and `f_end`, meaning we could skip `fluxible::flux_match`.
However, those are based on the start and end of the files, which would require a strict data collection routine, which was not done there (because there was no original plans to use fluxible since it did not exist).
The strategy will be to recreate the `field_record` input.
We can use the `t_start` and `t_finish` columns in the clean data for that, since they correspond to the cuts the team decided to apply.
Note that this is also an example of how fluxible can be used to process data in a non homogenous way, would that be necessary.

```{r reformat}
pftc5_data_raw <- pftc5_data_raw |> # will modify import7500 with this naming convention
  rename( # a bit of renaming to avoid confusions
    filename = "f_fluxid", # f_fluxid will be created by flux_match
    file_start = "f_start", # same
    file_end = "f_end"
  )

# downloading clean data
get_file("gs8u6",
         "c-flux",
         "PFTC3_Puna_PFTC5_Peru_2018_2020_Cflux.csv",
         "vignettes/ex_data/li7500")

pftc5_published_fluxes <- read_csv(
  "vignettes/ex_data/li7500/PFTC3_Puna_PFTC5_Peru_2018_2020_Cflux.csv",
  show_col_types = FALSE # quiet!
)

pftc5_cuts <- pftc5_published_fluxes |>
  mutate(
    date = ymd(paste(year, month, day))
  ) |>
  select(date, flux, site, treatment, plot_id, t_start, t_finish)

# t_start and t_finish are relative time
# we need to fetch datetime from the raw data

pftc5_record <- pftc5_data_raw |>
  mutate(
    plot_id = as.integer(str_extract(plot_id, "\\d+")),
    flux = case_when(
      trial == "r" ~ "Reco",
      trial == "p" ~ "NEE",
      trial == "p1" ~ "NEE1", # ideally a column "replicate" for that...
      trial == "p2" ~ "NEE2",
      trial == "p3" ~ "NEE3",
    ),
    date = as_date(f_datetime)
  ) |>
  select(file_start, date, flux, site, treatment, plot_id) |>
  distinct() |>
  drop_na(flux) |>
  left_join(pftc5_cuts, by = join_by(date, flux, site, treatment, plot_id)) |>
  mutate(
    start = file_start + t_start,
    end = file_start + t_finish
  ) |>
  select(!c(file_start, t_start, t_finish)) |> # to avoid confusion
  arrange(start)

# Now the meta data are in pftc5_record,
# we remove them from pftc5_data to avoid confusion
# flux match crashes if columns are both in raw data and record
# this will be corrected in next version

pftc5_data <- pftc5_data_raw |>
  select(!c(site, treatment, plot_id, trial, date))
```

Would that be necessary to process fluxes in a non homogenous way (manual cutting after visualization), the procedure would go as follow:

- use start and end from `licoread::import7500`
- process fluxes until visualization (`flux_fitting`, `flux_quality` and `flux_plot`)
- based on the plots, create a `record` file with metadata identifying the fluxes (`filename`), start and end (note that `flux_match` requires datetime format)
- include `flux_match` before `flux_fitting` in your workflow, and match the fluxes with the record file created
- adapt the record file if necessary and rerun until `flux_plot`
- once satisfied with cutting and fitting, run `flux_calc` as normally

## Processing with fluxible
<!-- Since the `licoread::import7500` already provides `f_fluxid`, `f_start` and `f_end`, we can skip `fluxible::flux_match` and directly fit with `fluxible::flux_fitting`. -->


```{r match, message = FALSE, warning = FALSE}
library(fluxible)
pftc5_match <- flux_match(pftc5_data,
                          pftc5_record,
                          f_datetime = f_datetime,
                          start_col = start,
                          end_col = end)
```

The wet air correction should happen here (coming soon).

### Exponential model

Fitting the model presented in @zhaoCalculationDaytimeCO22018.
```{r fitting_exp, message = FALSE, warning = FALSE}
pftc5_fits_exp <- flux_fitting(pftc5_match,
                           f_conc = `CO2 umol/mol`,
                           fit_type = "exp_zhao18",
                           start_cut = 0,
                           end_cut = 0)
```

Using `fluxible::flux_quality` to assess the quality of the dataset.
```{r flags_exp}
pftc5_flags_exp <- flux_quality(pftc5_fits_exp,
                                f_conc = `CO2 umol/mol`,
                                rsquared_threshold = 0.5)
```

```{r sites, include = FALSE, echo = FALSE, eval=FALSE}
pftc5_flags_exp |>
  dplyr::select(site) |>
  dplyr::distinct()
```

Then plotting (in an external file) for a visual check.
Because of the number of data, we will do it per site to make it lighter.
```{r plotting_exp, eval = FALSE}
pftc5_flags_exp |>
  dplyr::filter(site == "ACJ") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "ACJ_plot_exp")

pftc5_flags_exp |>
  dplyr::filter(site == "PIL") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "PIL_plot_exp")

pftc5_flags_exp |>
  dplyr::filter(site == "QUE") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "QUE_plot_exp")

pftc5_flags_exp |>
  dplyr::filter(site == "TRE") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "TRE_plot_exp")

pftc5_flags_exp |>
  dplyr::filter(site == "WAY") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "WAY_plot_exp")

pftc5_flags_exp |>
  dplyr::filter(f_quality_flag == "zero") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_ylim_upper = 500,
            f_ylim_lower = 350,
            y_text_position = 450,
            f_plotname = "zero_plot_exp")
```

If other data (PAR and co) need cleaning, that would happen here.
To "compress" environmental variables at a single value per flux, see the arguments `cols_ave`, `cols_sum`, and `cols_med` in `fluxible::flux_calc`.
To keep the raw data (gas concentration or anything else) in a nested column, see the `cols_nest` argument.

Now let's calculate the fluxes with `fluxible::flux_calc`.
```{r calc_exp}
pftc5_fluxes_exp <- flux_calc(pftc5_flags_exp,
                              slope_col = f_slope_corr,
                              temp_air_col = Temperature,
                              setup_volume = 2197,
                              atm_pressure = pressure_atm,
                              plot_area = 1.44,
                              conc_unit = "ppm",
                              flux_unit = "umol/m2/s",
                              cols_keep = c(
                                "site", "treatment", "date", "plot_id", "flux"
                              ))
```

### Linear model

Fitting a linar model.
```{r fitting_lm, message = FALSE, warning = FALSE}
pftc5_fits_lm <- flux_fitting(pftc5_match,
                           f_conc = `CO2 umol/mol`,
                           fit_type = "linear",
                           start_cut = 0,
                           end_cut = 0)
```

Using `fluxible::flux_quality` to assess the quality of the dataset.
```{r flags_lm}
pftc5_flags_lm <- flux_quality(pftc5_fits_lm,
                               f_conc = `CO2 umol/mol`,
                               rsquared_threshold = 0.5)
```



Then plotting (in an external file) for a visual check.
Because of the number of data, we will do it per site to make it lighter.
```{r plotting_lm, eval = FALSE}
pftc5_flags_lm |>
  dplyr::filter(site == "ACJ") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "ACJ_plot_lm")

pftc5_flags_lm |>
  dplyr::filter(site == "PIL") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "PIL_plot_lm")

pftc5_flags_lm |>
  dplyr::filter(site == "QUE") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "QUE_plot_lm")

pftc5_flags_lm |>
  dplyr::filter(site == "TRE") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "TRE_plot_lm")

pftc5_flags_lm |>
  dplyr::filter(site == "WAY") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_plotname = "WAY_plot_lm")

pftc5_flags_lm |>
  dplyr::filter(f_quality_flag == "zero") |>
  flux_plot(f_conc = `CO2 umol/mol`,
            print_plot = FALSE,
            output = "longpdf",
            f_ylim_upper = 500,
            f_ylim_lower = 350,
            y_text_position = 450,
            f_plotname = "zero_plot_lm")
```

If other data (PAR and co) need cleaning, that would happen here.
To "compress" environmental variables at a single value per flux, see the arguments `cols_ave`, `cols_sum`, and `cols_med` in `fluxible::flux_calc`.
To keep the raw data (gas concentration or anything else) in a nested column, see the `cols_nest` argument.

Now let's calculate the fluxes with `fluxible::flux_calc`.
```{r calc_lm}
pftc5_fluxes_lm <- flux_calc(pftc5_flags_lm,
                             slope_col = f_slope_corr,
                             temp_air_col = Temperature,
                             setup_volume = 2197,
                             atm_pressure = pressure_atm,
                             plot_area = 1.44,
                             conc_unit = "ppm",
                             flux_unit = "umol/m2/s",
                             cols_keep = c(
                               "site", "treatment", "date", "plot_id", "flux"
                             ))
```

## Comparison

Let's format the fluxible fluxes in a similar way.
```{r fluxible-format}
pftc5_fluxible_fluxes <- pftc5_fluxes_exp |>
  bind_rows(pftc5_fluxes_lm) |>
  mutate(
    fluxible_flux = -f_flux # the opposite convention was used for PFTC5
  ) |>
  drop_na(flux) # removing amb measurements
```

<!-- Now download and format PFTC5 fluxes. -->
<!-- In the clean data on OSF, both linear and nls fluxes are available.
Which one to choose is not explicit (but I am tired, do not quote me on that), so we will just take the one with the lowest aic for now. -->
```{r pftc5-fluxes}


pftc5_published_fluxes <- pftc5_published_fluxes |>
  mutate(
    date = ymd(paste(year, month, day))
  ) |>
  pivot_longer(
    c(linear_model, nls_model),
    values_to = "published_fluxes",
    names_to = "model"
  )
```

Putting them together:
```{r gathering}
pftc5_fluxes_comparison <- left_join(pftc5_published_fluxes,
                                     pftc5_fluxible_fluxes,
                                     by = join_by(date, flux,
                                                  site, treatment,
                                                  plot_id),
                                     relationship = "many-to-many")
```

```{r figure, echo=FALSE, fig.cap="Comparison of fluxes calculated with fluxible and published in Halbritter et al (2024).", message=FALSE, warning=FALSE, fig.width=15, fig.height=18}

pftc5_fluxes_comparison |>
  ggplot(aes(fluxible_flux, published_fluxes, color = model)) +
  theme_bw() +
  scale_color_manual(values = c(
        nls_model = "#0c9f0b",
        linear_model = "#bf28bd")
  ) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  geom_smooth(
    method = "lm"
  ) +
  facet_wrap(. ~ f_model, scales = "free", nrow = 2) +
  labs(
    title = "Reprocessing of PFTC5 flux data",
    x = "Fluxes calculated with Fluxible (exp_zhao18 and linear model respectively)",
    y = "Fluxes as published in the data paper",
    color = "Model (PFTC5)"
  )
  # geom_text(aes(label = f_fluxid), vjust = -0.5)
```

```{r cleaning, include = FALSE, eval = TRUE}
library(here)
unlink(here("building-material", "vignettes", "ex_data", "li7500"),
       recursive = TRUE)
```


