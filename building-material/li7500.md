...
The `licoread::import7500` function provides `f_fluxid`, `f_start` and
`f_end`, meaning we could skip `fluxible::flux_match`. However, those
...
strategy will be to recreate the `field_record` input. We can use the
`t_start` and `t_finish` columns in the clean data for that, since they
correspond to the cuts the team decided to apply. Note that this is also
an example of how fluxible can be used to process data in a non
homogenous way, would that be necessary.
...
<figure>
<img src="li7500_files/figure-gfm/figure-1.png"
alt="Comparison of fluxes calculated with fluxible and published in Halbritter et al (2024)." />
<figcaption aria-hidden="true">Comparison of fluxes calculated with
