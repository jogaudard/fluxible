...
The `exp_zhao18` is a mix of an exponential and linear model - thus fitting all fluxes independently from curvature - and includes $t_0$ as a fitting parameter (its implementation is described in `vignette("zhao18", package = "fluxible")`).
...
The `ambient_conc` and `error` arguments are used to detect measurements starting outside of a reasonable range (the mean of the three first gas concentration data points is used, independently from the focus window).
...
With `output = "longpdf"`, the plots are exported in a single pdf file (default width 29.7 cm and as long as needed).
This does not allow for printing and is aesthetically debatable, but it is much faster.
