...
The methods used is `fluxible` come from the litterature and are described in a publication recently accepted in *Methods in Ecology and Evolution* [@gaudardFluxiblePackageProcess2025].
[LI-COR](https://www.licor.com/), one of the largest manufacturers of infrared gas analyzers and soil respiration chamber systems, has been involved in the development and provided testing data (in the paper).

On the technical side, `fluxible` is developed following @wickham2023r, including fully automated formal tests with the `testthat` R package [@testthat2011].
...
The code coverage is maintained above 90% (meaning at least 90% of the code is subject to at least one test) through test driven development.
`fluxible` follows the `tidyverse` [@tidyverse2019] grammar, and all inputs and outputs are tibble S3 objects that can be used without restrictions with other packages and column names provided by the users are kept (at the exception of the `flux_plot` function, which is producing, well, a plot, and requires `fluxible` default column names).
...
## How does `fluxible` compare to other R packages?
...
- maximizing automation, making it time and resource efficient

### What about the `goFlux` R package?

`goFlux` [@rheaultGoFluxUserfriendlyWay2024] is not on CRAN and was therefore not included in the package comparison found in the paper, but since it has been presented alongside `fluxible` at several occasions, this question came up several times.
...
On the technical side, `goFlux` is hosted on GitHub only and does not seem to have any continuously integrated testing or a test driven development routine.
...
It has been broken by updates in the past.
